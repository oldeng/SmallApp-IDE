...
function getLastSessionFilesInfo(){
	let info = fs.readFileSync(path.join(User.getUserDataPath(), '/last_session/info.json'));  
	info = JSON.parse(info);
	return info;
}

function getLastSessionProjectInfo(){
	let info = fs.readFileSync(path.join(User.getUserDataPath(), '/last_session/project_info.json'));  
	info = JSON.parse(info);
	return info;
}

function writeLastSessionFilesInfo(data){
	fs.writeFileSync(path.join(User.getUserDataPath() , '/last_session/info.json'), data);
}

function writeLastSessionProjectInfo(data){
	fs.writeFileSync(path.join(User.getUserDataPath() , '/last_session/project_info.json'), data);
}
...
function checkLastSessionFilesAndOpen(){
	...
	if(Object.keys(info).length > 0){
	    ...
	    for (let i in info){
	        let data = fs.readFileSync(path.join(User.getUserDataPath(), info[i].current_path), 'utf-8');
	        //openFile(data, info[i].original_path);
	        if('#' + file.id != i){
	            fs.unlinkSync(path.join(User.getUserDataPath(), info[i].current_path));
	        }
	    }
	}else{
	    //newFile();
...
function changeLastSessionFilesContent(fileID, data){
	...
	fs.writeFile(path.join(User.getUserDataPath(), info['#' + fileID].current_path), data, function(error){
...
function makeNewLastSessionFile(filecount, filepath, data){
	...
	fs.writeFile(path.join(User.getUserDataPath(), info['#new' + filecount].current_path), data, function(error){
        if(error) throw error;
    });
...
function deleteLastSessionFile(fileID,){
	let info = getLastSessionFilesInfo();
	if(info['#' + fileID]){
        fs.unlinkSync(path.join(User.getUserDataPath(), info['#' + fileID].current_path));
    }
	delete info['#' + fileID];
	info = JSON.stringify(info, null, 2);
    writeLastSessionFilesInfo(info);
